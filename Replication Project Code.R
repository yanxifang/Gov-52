...
# use same data from table 4
# manipulate data
table6_data <- pdata.frame(subset(ward_data, (lab.borough02==1 & lab.borough06 == 1)),
                           index = c("borough", "ward"))
table6_data <- table6_data %>%
  mutate(log_council11 = log(council11)) %>%
  mutate(log_council01 = log(council01))
table6_weak <- subset(table6_data, labseats06 < 3 | is.na(labseats06))
table6_strong <- subset(table6_data, labseats06 == 3)

# simple models (no interactions or control variables)
table6_a <- plm(log_council11 ~ log_council01 + log_crime_z + log_medincome_z,
                model = "within", data = table6_data)
table6_b <- plm(log_council11 ~ log_council01 + log_crime_z + log_medincome_z,
                model = "within", data = table6_weak)
table6_c <- plm(log_council11 ~ log_council01 + log_crime_z + log_medincome_z,
                model = "within", data = table6_strong)

# full models (with control variables, similar to model 3 in figure 3)
table6_d <- plm(log_council11 ~ log_council01 + log_crime_z + log_medincome_z +    #basic model
                  unemp01 + p.inc.supp01 + black01 + asian01 +             #demographics
                  private01 + log_sales01 + log_price01 +                  #housing mkt. indicators
                  log_dens01 + log_hholds01 + over.private01 +             #housing demand indicators
                  noheat01 + ownbath01 + block01 + over.council01 +        #characteristics of council hsg.
...
table6_e <- plm(log_council11 ~ log_council01 + log_crime_z + log_medincome_z +    #basic model
                  unemp01 + p.inc.supp01 + black01 + asian01 +             #demographics
                  private01 + log_sales01 + log_price01 +                  #housing mkt. indicators
                  log_dens01 + log_hholds01 + over.private01 +             #housing demand indicators
                  noheat01 + ownbath01 + block01 + over.council01 +        #characteristics of council hsg.
...
table6_f <- plm(log_council11 ~ log_council01 + log_crime_z + log_medincome_z +    #basic model
                  unemp01 + p.inc.supp01 + black01 + asian01 +             #demographics
                  private01 + log_sales01 + log_price01 +                  #housing mkt. indicators
                  log_dens01 + log_hholds01 + over.private01 +             #housing demand indicators
                  noheat01 + ownbath01 + block01 + over.council01 +        #characteristics of council hsg.
...
stargazer(table6_a, table6_b, table6_c, table6_d, table6_e, table6_f,
          dep.var.caption = "Log of Council Housing Concentration in 2011",
          se = list(sqrt(diag(vcovHC(table6_a, cluster = "group"))), 
                    sqrt(diag(vcovHC(table6_b, cluster = "group"))), 
                    sqrt(diag(vcovHC(table6_c, cluster = "group"))), 
                    sqrt(diag(vcovHC(table6_d, cluster = "group"))), 
                    sqrt(diag(vcovHC(table6_e, cluster = "group"))), 
                    sqrt(diag(vcovHC(table6_f, cluster = "group")))), 
          style = "apsr", 
          digits = 3, 
          omit.stat = "f",
          out = "Table-6.tex")
